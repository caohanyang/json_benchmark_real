...
work=paste("//home//caohanyang//MEAN//Results//resultCompute0511")
setwd(work)
times=10000
timesL=100
...
#A0
SS0<-read.csv(paste("small-P0.1-A0.csv"), check.names=FALSE, header = FALSE)
SM0<-read.csv(paste("small-P0.21-A0.csv"), check.names=FALSE, header = FALSE)
SL0<-read.csv(paste("small-P0.49-A0.csv"), check.names=FALSE, header = FALSE)

MS0<-read.csv(paste("medium-P0.1-A0.csv"), check.names=FALSE, header = FALSE)
MM0<-read.csv(paste("medium-P0.2-A0.csv"), check.names=FALSE, header = FALSE)
ML0<-read.csv(paste("medium-P0.37-A0.csv"), check.names=FALSE, header = FALSE)

LS0<-read.csv(paste("large-P0.1-A0.csv"), check.names=FALSE, header = FALSE)
LM0<-read.csv(paste("large-P0.2-A0.csv"), check.names=FALSE, header = FALSE)
LL0<-read.csv(paste("large-P0.39-A0.csv"), check.names=FALSE, header = FALSE)
...
axis(1,at=bar[seq(3, length(bar), 5)],labels=label, lwd=0)
legend("topleft", legend = legs, fill = cols, box.col = "transparent",cex=0.9)
abline(a = NULL, b = NULL, h = 500, v = NULL, reg = NULL)
box()
dev.off() 
